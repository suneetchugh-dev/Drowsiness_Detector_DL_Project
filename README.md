...
image/video) and warns when the driver is **drowsy**, **yawning** or
**distracted**. Built with PyTorch, OpenCV, scikit-learn and YOLOv8 — using only
the standard workshop library set.
...
- **One-click launcher**: double-click `Drowsiness_Detector.bat` to open a
  Tkinter GUI with all three modes
...
| DISTRACTED | Face out of frame / off-centre |
...
2. **Easiest way** — double-click `Drowsiness_Detector.bat` and pick a mode.
...
frame ──► Haar cascade (face) ──► geometric eye/mouth crops
   ...
   YOLOv8 person check (optional) │
                   ...
                   MouthCNN ─► yawn / no yawn (gated by mouth-openness)
                                  │
                   temporal counters ──► DROWSY / YAWNING / DISTRACTED / SAFE
```
...
- `gui.py` — Tkinter launcher used by `Drowsiness_Detector.bat`
